...
class ResponsesBody(BaseModel):
    ...
    def __init__(self, **data):
        ...
        if hasattr(self, 'model') and isinstance(self.model, str):
            ...
            if self.model.startswith("openai_responses_manifold."):
                self.model = self.model.removeprefix("openai_responses_manifold.")
            if self.model.startswith("openai_responses."):
                self.model = self.model.removeprefix("openai_responses.")
...
class Pipe:
    ...
    async def _run_nonstreaming_loop(
        ...
        valves: Pipe.Valves,                                        # Contains config: MAX_TOOL_CALLS, API_KEY, etc.
    ...
    async def send_openai_responses_streaming_request(
        ...
        payload immediately.
        """
        # Debug: print the outgoing request payload
        print("[DEBUG] Streaming request payload to OpenAI /v1/responses:\n", json.dumps(request_body, indent=2, ensure_ascii=False))
    ...
    async def send_openai_responses_nonstreaming_request(
        ...
        # Debug: print the outgoing request payload
        print("[DEBUG] Non-streaming request payload to OpenAI /v1/responses:\n", json.dumps(request_params, indent=2, ensure_ascii=False))
